   ...
   <div class="row" style="margin-top: 20px;">
      ...
      <div class="panel">
                <div class="panel-heading">
                  ...
                  <a href="<?php echo U("Dustbin/add"); ?>" class="btn btn-info btn-gradient pull-right"><span class="glyphicons glyphicon-plus"></span> 添加设备</a>
                </div>
                <form action="" method="post">
                <div class="panel-body">
                  <h2 class="panel-body-title">设备列表</h2>
                  <table class="table table-striped table-bordered table-hover dataTable">
                      <tr class="active">
                        <th class="text-center" width="100"><input type="checkbox" value="" id="checkall" class="dels"> 全选</th>
                        <th>编号</th>
                        <th>经度</th>
                        <th>纬度</th>
                        <th>位置</th>
            ...
            <?php if(is_array($list)): foreach($list as $key=>$value): ?><tr class="success">
                        <td class="text-center"><input type="checkbox" value="<?php echo ($value["id"]); ?>" name="idarr[]" class="cbox" data-id="<?php echo ($value["id"]); ?>"></td>
                        <td><?php echo ($value["phone"]); ?></td>
                        <td><?php echo ($value["lng"]); ?></td>
                        <td><?php echo ($value["lat"]); ?></td>
                        <td><?php echo ($value["address"]); ?></td>
                        <td><?php echo $value['full']==1?'已满':'未满'; ?></td>
                        <td>
                           <?php if($value['warning']==1){ ?>
                           <img src="/Dustbin/Uploads/OTOHTW74[3BPN7IU1QX])IE.png" width="50" height="50">
                           <?php }else{?>
                           <img src="/Dustbin/Uploads/8EIS[(`]8(H_VYHIVNA}P~C.png" width="50" height="50">
                           <?php }?> 
                        </td>
                        <td>
                          <div class="btn-group">
                            <a href="<?php echo U('Dustbin/edit',array('id'=>$value['id']));?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
                            <a onclick="return confirm('确定要删除吗？');" href=" <?php echo U('Dustbin/del',array('id'=>$value['id']));?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
                          </div>
